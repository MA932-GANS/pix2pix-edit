...
def main():
    ...
    for i in sets:
        ...
        g = cv2.cvtColor(cv2.imread(os.path.join(src_dir, 'R2G_%d-outputs.png'%(i))),cv2.COLOR_BGR2RGB).astype(np.uint8)
        out = r + g

        tmp1 = Image.fromarray(out, 'RGB')
        
        tmp1.save('Artificial Cells/fake_cell_%d.png'%(i), format='PNG')
